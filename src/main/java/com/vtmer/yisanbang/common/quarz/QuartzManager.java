...
public class QuartzManager {
    ...
    /**
     * 使用SimpleScheduleBuilder添加一个定时任务
     * @param jobName 任务名
     * @param cls     任务
     * @param minutesTime 时间，单位：分钟
     */
    public void addJob(String jobName, Class<? extends Job> cls, int minutesTime) {
        try {
            JobDetail jobDetail = JobBuilder.newJob(cls).withIdentity(jobName).storeDurably().build();    //用于描叙Job实现类及其他的一些静态信息，构建一个作业实例
            SimpleScheduleBuilder scheduleBuilder = SimpleScheduleBuilder.simpleSchedule()
                    .withIntervalInMinutes(minutesTime)
                    .repeatForever();
